...
// Maximum number of processes is 100 due to array size
struct thread threads[100];

// Finds thread using pid
struct thread *
thread_find(pid_t pid)
{
        if (numthreads <= pid || pid < 1 || threads[pid] == NULL) {
                return NULL;
        }

        return threads[(int)pid];
}
...
thread_create(const char *name)
{
        ...
        thread->t_lock = NULL;
                ...
                while(threads[current]!=NULL) {
                        current++;
                }
        thread->t_pid = current;
        thread->t_ppid = -1;
...
thread_destroy(struct thread *thread)
{
        ...
        assert(thread->t_cwd==NULL);
                ...
                if((int) thread->t_pid < current) {
                        current = (int) thread->t_pid;
                }

        if (thread->t_stack) {
                kfree(thread->t_stack);
        }
                threads[thread->t_pid] = NULL;
        kfree(thread->t_name);
                kfree(thread->t_lock);
                kfree(thread->t_pid);
                kfree(thread->t_ppid);
...
thread_bootstrap(void)
{
        ...
        if (zombies==NULL) {
                ...
                // Just to make sure the first created thread has pid 0
                numthreads = 0;
                current = 0;
                parrent = 0;
        /*
...
thread_exit(void)
{
        ...
        if (curthread->t_cwd) {
                ...
                if((int) curthread->t_pid < current) {
                        current = (int) curthread->t_pid;
                }
                // Extra clean up for ASST2
                curthread->t_lock = NULL;
                curthread->t_pid = NULL;
                curthread->t_ppid = NULL;
                threads[curthread->t_pid] = NULL;
